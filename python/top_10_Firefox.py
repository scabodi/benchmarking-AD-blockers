...
f_out_dati = '/home/sara/Scrivania/docs/outputs_dati_firefox'
# file di output delle misure dei tempi
f_out_tempi = '/home/sara/Scrivania/docs/outputs_tempi_firefox'
# file degli errori
f_err = '/home/sara/Scrivania/docs/failures_firefox'
...
def calcola_media(array):  # cercare i due max e i due min e toglierli e fare la media dei restanti
    ...
    # stand_err = stats.sem(elenco_ridotto)  # calcolo lo standard error
    # diff = elenco[len(elenco_ridotto) - 1] - elenco_ridotto[0]  # range calcolato come diff di max e min
    # fare la media dei rimanenti
    media = statistics.mean(elenco_ridotto)
    # prendo il max e il min e calcolo la differenza dalla media e li salvo in due liste diverse
    up = massimo - media
    down = media - minimo
    # media = round(media, 2)
    # stand_err = round(stand_err, 2)
...
# noinspection PyShadowingNames,PyDeprecation
def set_firefox(profilo):
    nome_profilo = ""
    if profilo not in range(1, 4):
        ...
        if profilo == 1:
            ...
            nome_profilo = 'adblock_plus'
        elif profilo == 3:
            nome_profilo = 'ublock'
        # imposto il driver
        profile = webdriver.FirefoxProfile('/home/sara/.mozilla/firefox/'+nome_profilo)
...
def visita(profilo, driver, sito, nome_sito, id_sito):
    if profilo not in range(1, 4):
        ...
        f_sito = "/home/sara/Scrivania/output_singoli/" + nome_sito + "_firefox"
        if profilo == 1:
            doc = open(f_sito, "w")
        else:
            doc = open(f_sito, "a")
        ...
        for i in range(10):  # qui faccio tutte le get per un singolo sito ripetute
            ...
            try:
                ...
                # clear_cache(driver)
            except NoSuchWindowException:  # riprovo
                # creo un nuovo har
                proxy.new_har(nome_sito + str(i))
                # definisco il momento d'inizio
                start = time.time()
...
def plot_multiplo(data, tempi, errori):
    ...
    fig.suptitle("Test 10 siti - FIREFOX", size=20)
    ...
    plt.savefig("/home/sara/Scrivania/docs/plots/tris_firefox.png")
    plt.show()
